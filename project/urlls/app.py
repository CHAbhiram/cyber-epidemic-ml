...
import hashlib
import tldextract
import joblib  # Assuming you're using joblib for the pipeline
# Import or define all the functions from your code above here (e.g., get_numerical_values, model_predict)

# Load your machine learning model pipeline
pipeline = joblib.load('BestModel_ExtraTreesClassifier.joblib')  # Adjust the file name/path as needed
...
def predict():
    ...
    try:
        # Generate numerical features for the given URL
        numerical_values = get_numerical_values(url)
        features = np.array(list(numerical_values.values())).reshape(1, -1)
        
        # Predict using the model
        prediction_int, prediction_label = model_predict(url)
